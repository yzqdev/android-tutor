...
public class BackgammonActivity extends AppCompatActivity {
    ...
    private void initGame() {
        ...
        this.ai = new EasyAi();
        int icount = 0;
        System.out.println("backgrammon的构造函数");
        //遍历所有的五连子可能情况的权值
        //横  
        for (int i = 0; i < 15; i++)
            for (int j = 0; j < 11; j++) {
...
//        System.out.println(Arrays.deepToString(this.combo));
    ...
    private void showWinner(int id) {//显示结束信息
        ...
        builder.setNegativeButton(this.getResources().getText(R.string.view_chess), null);
        builder.show();
    }
    ...
    /**
     * setPiece方法 前置条件：已经检查过该格放入棋子不会造成冲突，并且已经检查是否轮到该色棋子下后置条件：将指定棋子放入指定格子内
     *
     * @param x
     * @param y
     * @param id
