...
public class EasyAi {
    ...
    private boolean[][][] playerTable = new boolean[15][15][572];  //玩家的可能性
    ...
    //临时变量
    private int computerGrade, playerGrade;
    private int icount,m,n;
    private int mat,nat,mde,nde;
    
    public EasyAi(){
        System.out.println("人工智能的构造函数");
    	initGame();
    }
    ...
    public Coordinate comTurn(Coordinate c){     //找出电脑（白子）最佳落子点  
        ...
        for(int i=0;i<15;i++)     //遍历棋盘上的所有坐标  
	             ...
	             for(int i=0;i<15;i++)  
	                 for(int j=0;j<15;j++)  
	                     if(this.board[i][j] == 0){  //找出棋盘上可落子点的黑子白子的各自最大权值，找出各自的最佳落子点  
	                         if(this.cgrades[i][j]>=this.computerGrade){
	                             this.computerGrade = this.cgrades[i][j];
	                             this.mat = i;  
	                             this.nat = j;  
	                         }  
	                         if(this.pgrades[i][j]>=this.playerGrade){
	                             this.playerGrade = this.pgrades[i][j];
	                             this.mde = i;  
	                             this.nde = j;  
	                         }  
	                     }  
	             if(this.computerGrade >=this.playerGrade){   //如果白子的最佳落子点的权值比黑子的最佳落子点权值大，则电脑的最佳落子点为白子的最佳落子点，否则相反
	                 m = mat;  
	                 n = nat;  
	             }else{  
	                 m = mde;  
         ...
         this.computerGrade = 0;
         this.playerGrade = 0;
         this.board[m][n] = 1;  //电脑下子位置     
         for(int i=0;i<572;i++){  
             if(this.ctable[m][n][i] && this.win[0][i] != -1)  
                 this.win[0][i]++;     //给白子的所有五连子可能的加载当前连子数  
             if(this.playerTable[m][n][i]){
                 this.playerTable[m][n][i] = false;
                 this.win[1][i]= -1;  
             } 
         }
         Coordinate co = new Coordinate(m,n);
        System.out.println("记录这里玩家和电脑的坐标");
        System.out.println(Arrays.deepToString(this.win));
	...
	private void setPlayersPiece(Coordinate c) {
		...
		if(this.board[m][n] == 0){     
            this.board[m][n] = 2;   
            for(int i=0;i<572;i++){  
                if(this.playerTable[m][n][i] && this.win[1][i] != -1)
